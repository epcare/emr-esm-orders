/**
 * Order-to-Result Field Mapping Utility
 *
 * This module provides utilities to transform OpenMRS Order data
 * into initial form values for Procedure and Imaging result forms.
 *
...
/**
 * Form field types for result forms
 */
export interface ProcedureResultFormDefaults {
...
export interface ImagingResultFormDefaults extends ProcedureResultFormDefaults {
 ...
 */
export function orderToProcedureFormDefaults(
 ...
 * Includes all procedure defaults plus imaging-specific observation fields
 * @param order - The OpenMRS Order object
 * @param procedure - Existing procedure data (for editing)
 * @returns Form default values object
 */
export function orderToImagingFormDefaults(
  ...
  const baseDefaults = orderToProcedureFormDefaults(order, procedure);
...
/**
 * Order Field → Form Field Mapping Reference
 *
 * Procedure Result Form:
 * ┌─────────────────────────┬────────────────────────────────────────────┐
 * │ Order Field              │ Form Field                                 │
 * ├─────────────────────────┼────────────────────────────────────────────┤
 * │ concept.uuid             │ procedureCoded                             │
 * │ bodySite.uuid            │ bodySite (object) → bodySite (string UUID)   │
 * │ bodySite.display         │ Display value for dropdown               │
 * │ laterality               │ laterality dropdown                         │
 * │ instructions             │ notes (combined with commentToFulfiller)    │
 * │ commentToFulfiller        │ notes (combined with instructions)        │
 * │ urgency                  │ urgency (display only)                      │
 * │ orderReasonNonCoded      │ orderReason / clinicalIndication           │
 * │ uuid                     │ _orphanedData.procedureOrder                │
 * │ accessionNumber          │ _orphanedData.accessionNumber (imaging)    │
 * │ dateActivated            │ Not directly used (reference only)         │
 * ├─────────────────────────┼────────────────────────────────────────────┤
 * │ Procedure Fields         │ Form Field                                 │
 * ├─────────────────────────┼────────────────────────────────────────────┤
 * │ procedureType.uuid       │ procedureType                               │
 * │ startDateTime            │ startDateTime                               │
 * │ endDateTime              │ endDateTime                                 │
 * │ status.uuid              │ status (enum → Concept UUID mapping)       │
 * │ outcomeCoded.uuid        │ outcomeCoded (enum → Concept UUID mapping)  │
 * │ estimatedStartDate       │ estimatedStartDate                           │
 * │ duration                 │ duration                                    │
 * │ durationUnit.uuid        │ durationUnit                                │
 * │ notes                    │ notes (combined with order fields)          │
 * └─────────────────────────┴────────────────────────────────────────────┘
 *
 * Imaging Result Form (includes all above plus):
 * ┌─────────────────────────┬────────────────────────────────────────────┐
