...
// Import ConceptSearchField - this is from esm-patient-procedures-app
// For now, we'll use a local implementation based on the useConceptSearchField hook
// TODO: Consider creating a shared component library
...
type ProcedureResultFormComponentProps = {
  closeWorkspace: () => void;
  procedure?: Procedure;
  order?: Result;
  patientUuid: string;
...
const ProcedureResultFormComponent: React.FC<ProcedureResultFormComponentProps> = ({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await saveProcedureResult(payload, order?.uuid, encounterUuid, useOrderEncounter, config);
      showSnackbar({
        kind: 'success',
      ...
      <div className={styles.formContainer}>
        <Stack gap={5}>
          ...
          <DurationField
            ...
            label={t('procedureDuration', 'Procedure duration')}
          />
          ...
          <FormGroup legendText={<RequiredFieldLabel label={t('procedureOutcome', 'Procedure outcome')} />}>
            <Controller
              name="outcomeCoded"
              control={control}
              render={({ field, fieldState }) => (
...
export default ProcedureResultFormComponent;
